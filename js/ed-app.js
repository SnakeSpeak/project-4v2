...
const statePop = d3.json("https://yleep4flask.azurewebsites.net/api/v1.0/population")


function displayPop(state) {
    //parse out api data
    d3.json("https://yleep4flask.azurewebsites.net/api/v1.0/population")
    .then(data => {
...
function createLightingBarGraph(state) {
    //parse out api data
    d3.json("https://yleep4flask.azurewebsites.net/api/v1.0/lighting_conditions").then((data) => {
        //filter to only see selected state data
        let value = data.filter(result => result.state == state)
